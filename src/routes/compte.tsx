...
export const Route = createFileRoute("/compte")({
  ssr: false,
  head: () => ({ meta: [{ title: "Espace client — CONETEC" }, { name: "description", content: "Votre compte, vos commandes et votre historique d'achats CONETEC." }] }),
...
function ComptePage() {
  ...
  const fProfile = useServerFn(getMyProfile);
  const fOrders = useServerFn(getMyOrders);
  const profile = useQuery({ queryKey: ["my-profile"], queryFn: () => fProfile(), enabled: ready });
  const orders = useQuery({ queryKey: ["my-orders"], queryFn: () => fOrders(), enabled: ready });
  ...
  const totalSpent = list.filter((o: any) => o.status === "paid" || o.status === "delivered")
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-3">
            <div className="min-w-0">
              ...
              <h1 className="mt-1 truncate text-2xl font-black sm:text-3xl">
                Bonjour {profile.data?.full_name?.split(" ")[0] ?? "👋"}
              </h1>
        ...
        <Card className="p-0">
          ...
          {list.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted-foreground">
              Aucune commande pour le moment. <Link to="/boutique" className="font-semibold text-accent hover:underline">Voir la boutique</Link>
            </div>
          ) : (
