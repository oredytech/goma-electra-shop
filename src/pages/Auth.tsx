...
export default function AuthPage() {
  ...
  async function handleGoogle() {
    ...
    else if (!r.redirected) navigate("/", { replace: true });
  }
    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="hidden bg-gradient-hero p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div>
          ...
          <p className="mt-3 max-w-md text-white/80">Connectez-vous pour suivre vos commandes, vos factures et accéder à l'espace administration.</p>
        </div>
          ...
          <Tabs defaultValue="signin">
            ...
            <TabsContent value="signin">
              <form onSubmit={handleSignIn} className="space-y-4 pt-4">
                ...
                <Button type="submit" disabled={loading} className="w-full bg-gradient-brand text-brand-foreground">{loading ? "Connexion..." : "Se connecter"}</Button>
              </form>
            </TabsContent>
            <TabsContent value="signup">
              <form onSubmit={handleSignUp} className="space-y-4 pt-4">
                ...
                <Button type="submit" disabled={loading} className="w-full bg-gradient-brand text-brand-foreground">{loading ? "Création..." : "Créer le compte"}</Button>
              </form>
            </TabsContent>
          </Tabs>
