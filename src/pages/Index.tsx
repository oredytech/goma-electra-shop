...
export default function HomePage() {
  ...
  const products = useQuery({ queryKey: ["home-products"], queryFn: () => listProducts({ data: { limit: 8 } }) as Promise<any[]> });
  ...
  useEffect(() => {
    const t = window.setInterval(() => setSlide((s) => (s + 1) % services.length), 4200);
  ...
  function go(e?: React.FormEvent) {
    e?.preventDefault();
    const search = q.trim();
    if (search) navigate(`/boutique?q=${encodeURIComponent(search)}`);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative isolate overflow-hidden bg-[oklch(0.18_0.06_264)] text-white">
        <div className="absolute inset-0 -z-10">
          {services.map((s, i) => (
            <div key={s.title} aria-hidden={i !== slide} className={`absolute inset-0 h-full w-full transition-opacity duration-1000 ${i === slide ? "opacity-100" : "opacity-0"}`}>
        ...
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:py-28">
          <div className="max-w-2xl">
            ...
            <div className="relative mt-5 h-14 max-w-lg overflow-hidden">
              {services.map((s, i) => (
                <div key={s.title} className={`absolute inset-0 flex items-center gap-3 transition-all duration-700 ${i === slide ? "translate-y-0 opacity-100" : i === (slide - 1 + services.length) % services.length ? "-translate-y-full opacity-0" : "translate-y-full opacity-0"}`} aria-hidden={i !== slide}>
            ...
            <div className="mt-2 flex gap-1.5">
              {services.map((_, i) => (
                <button key={i} aria-label={`Service ${i + 1}`} onClick={() => setSlide(i)} className={`h-1.5 rounded-full transition-all ${i === slide ? "w-6 bg-accent" : "w-1.5 bg-white/30"}`} />
              ))}
            </div>
            <form onSubmit={go} className="mt-7 flex w-full max-w-xl flex-wrap gap-2">
              <div className="relative min-w-0 flex-1">
                <Search className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-muted-foreground" />
                <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher : câble, disjoncteur, antenne…" className="h-12 rounded-full border-0 bg-white pl-12 pr-4 text-base text-foreground shadow-lg placeholder:text-muted-foreground" />
              </div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-2 gap-y-2 text-xs sm:text-sm">
              ...
              {popularTags.map((t) => (
                <Link key={t} to="/boutique" state={{ q: t }} className="rounded-full border border-white/30 bg-white/5 px-3 py-1 text-white/90 transition hover:border-accent hover:bg-accent/20">
                  {t}
                </Link>
              ))}
            </div>
      ...
      <section id="categories" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:py-20">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {(cats.data ?? []).slice(0, 6).map((c) => (
            <Link key={c.id} to="/boutique" state={{ cat: c.slug }} className="group rounded-xl border border-border bg-card p-3 transition hover:-translate-y-0.5 hover:border-accent hover:shadow-accent">
      ...
      <section className="bg-secondary/30">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:py-20">
          ...
          {items.length > 0 && (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {items.map((p) => (
                <Link key={p.id} to={`/produit/${p.slug}`} className="group flex flex-col overflow-hidden rounded-xl border border-border bg-card transition hover:-translate-y-0.5 hover:border-accent hover:shadow-accent">
